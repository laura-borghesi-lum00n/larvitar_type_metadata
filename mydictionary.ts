...
{
    let dicomtag : string=key;
    //dicomtag is key of metadata object (see createcustomImage in Larvitar for example)
    let VR = findVRfromTAG(dicomtag);
    //check the metadatatype from VR and cast new variable metadatavalue with correct type
    if (listVR_string.includes(VR))
    {
        let metadatavalue = metadata[dicomtag] as string; 
        //break or call function to be used on string types 
    }

    else if (listVR_number.includes(VR))
    {
        //let metadatavalue = metadata[dicomtag] as number; 
        //break or call function to be used on number types
    }
}
...
//it is not possible to create a mixed type object "a priori" unless using "any" 
//it is not possible to change a variable type afterwards unless using casting
//it is not possible to create a function that returns different types unless using "any" or union types

//This script can be used when we know the function will act on a certain type and we can select 
//on which metadata it can act on basing on its type.

//example: we have a function that acts on numbers and we check if the metadata is a number, then we proceed, 
//on the other hand, we break out of the function.
...
let dicomtag="3008 0010";
const VR = findVRfromTAG(dicomtag) as keyof dictVRtotype;
type MetadataValue = dictVRtotype[VR];
*/



/*2ND TRY: CONDITIONAL TYPES->FAILED
let dicomtag="3008 0010";
const VR = findVRfromTAG(dicomtag);
//example
type MetadataValue = (VR==="UV") ? number : string; */
