...
{
	let array_tags=[];
	let array_grouptag=Object.keys(dict);
	for(const i in array_grouptag)
	{  
	
		let grouptag=array_grouptag[i];
    	let subdict=dict[grouptag];
		let array_elementtag=Object.keys(subdict);

		for(const j in array_elementtag)
		{
			let elementtag=array_elementtag[j];
    		let tag_value=grouptag+" "+elementtag;
...
function findVRfromTAG(dicomtag)
{
	
    const dicomtag_array=dicomtag.split(" ");
    const group_tag=dicomtag_array[0];
    const element_tag=dicomtag_array[1];
    const VR=dict[group_tag][element_tag][0];
    return VR;
}

function createtotaldict ()
{
    ...
    for(var i=0; i<array_tags.length; i++)
        {
            ...
            let tag_value=array_tags[i];
            let VR=findVRfromTAG(tag_value);
            let type_value=MetaDataTypes[VR];
		    let obj={
				tag_value,
				type_value
			}
			string= string+"\""+tag_value+"\""+ " : "+ type_value+"<br>";
            result.push(obj);
        }
...
let string=createtotaldict();
document.getElementById("dict_total").innerHTML = string;
