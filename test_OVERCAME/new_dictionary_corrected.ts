...
function findVRfromTAG(dicomtag : string): keyof MetaDataTypes
{
    ...
    return VR as keyof MetaDataTypes;
}
...
/*export interface DataSet {
   ...
   /* const parseTag = function (dataSet, propertyName //thetag
    , element) {
    // GET VR
    var tagData = dataSet.elements[propertyName] || {}; //the metadata
    var vr = tagData.vr; 
    if (!vr) {
    ...
    if (isStringVr(vr)) {
        ...
        //Finds the element for tag and returns a string if it exists and has data.
...
function newParseTag<T>(tag: string, valueIn: string) {
    let VR=findVRfromTAG(tag);
	const value = valueIn as MetaDataTypes[typeof VR];
	// ... tutta la logica che sta attualmente in parseTag
	return value as T;
  }
  
  // con tag dinamico, come accade in parseDataSet
  const propertyName = "00100010";
  let VR=findVRfromTAG(propertyName);
  let valueIn = "13";
  newParseTag<MetaDataTypes[typeof VR]>(tag,  valueIn);
  let valueOut = "Pippo";
  

  export default {findVRfromTAG};